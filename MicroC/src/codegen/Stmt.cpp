...
Value *AST::Stmt::If::codegen(LLVMBundle &bundle) const {
  ...
  // true_block = bundle.builder.GetInsertBlock(); // update for PHI
  ...
  if (block_else) {
    ...
    // false_block = bundle.builder.GetInsertBlock();  // update for PHI
  }
...
Value *AST::Stmt::Return::codegen(LLVMBundle &bundle) const {
  ...
  if (this->value.has_value()) {
    ...
    auto [return_val, return_typ] = bundle.access(this->value.value().get());
